...
pub fn draw_ui(f: &mut Frame, app: &App) {
    ...
    let help_text = match app.input_mode {
        InputMode::Normal => {
            "Use 'h'/'l' to navigate columns | 'j'/'k' to navigate tasks | 'm' to move task | 'gc' to jump to column | 'gt' to jump to task | Space+ac to add column | Space+at to add task | Space+dt to delete task | Space+dc to delete column | Space+b for board selection | Ctrl+S to save | 'q' to quit"
        }
